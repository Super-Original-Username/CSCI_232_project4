...
public class Tests {
    ...
    public void testIfBothIllegal() { //Checks if the right exception is thrown if both the array and initial value are illegal
        int array[] = {};
        int in = 0;
        Exception e = assertThrows(IllegalArgumentException.class, () -> {Params.coinChange(array,in);});
    ...
    public void testIfInitLessThanOne(){ // Checks if the program throws the right exception when the input value is less than one
        int[] arr = {1,5,10,25};
        int in = -1;
        Exception e = assertThrows(IllegalArgumentException.class, () -> {Params.coinChange(arr,in);});
    ...
    public void seeIfItWorks(){ // Checks if it does the thing
        ...
        toCompare.add(1);
        int in = 42;
        assertEquals(toCompare, Params.coinChange(arr,in));
    }
}
